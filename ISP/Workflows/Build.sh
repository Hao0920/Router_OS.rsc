...
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/all_cn.txt) ; do
...
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/unicom_cnc.txt) ; do
  echo "add list=china_unicom address=$address"
done
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/cmcc.txt) ; do
  echo "add list=china_mobile address=$address"
done
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/crtc.txt) ; do
  echo "add list=china_tietong address=$address"
done
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/cernet.txt) ; do
  echo "add list=china_education_and_research_network address=$address"
done
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/gwbn.txt) ; do
  echo "add list=great_wall_broadband_network address=$address"
done
for address in $(cat ./ISP/Cache/Upstream/Clang.CN/othernet.txt) ; do
...
### 清除缓存 ###
# rm -rf ./ISP/Cache/Upstream/Clang.CN
