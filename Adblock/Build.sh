...
### 整理 ###
find ./Adblock/Cache/Source -type f -name "*.txt" | xargs cat > ./Adblock/Cache/Build/1.txt
# 删除注释行
...
# 删除空格行
sed -e '/^$/d' ./Adblock/Cache/Build/6.txt > ./Adblock/Cache/Build/7.txt
# 去重
sort -u ./Adblock/Cache/Build/7.txt > ./Adblock/Cache/Build/8.txt

### 构建 ###
{
  echo "/ip dns static"
  echo "remove [find address=240.0.0.1]"
  for name in $(cat ./Adblock/Cache/Build/8.txt) ; do
